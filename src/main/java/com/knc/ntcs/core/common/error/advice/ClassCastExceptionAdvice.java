...
public class ClassCastExceptionAdvice {
  ...
  public ResponseEntity<?> ClassCastException(ClassCastException ex,
    ...
    BizExceptionUtils.fillInStackTrace(fillInStackTrace,"ClassCastExceptionAdvice", ex.fillInStackTrace().toString());

    List<ErrorMsgDTO> error = new ArrayList<ErrorMsgDTO>();

    CustomMessageDTO customMessageDTO = CustomMessageDTO.builder().build();
    BizExceptionUtils.getCustomMessage(customMessageDTO, messageSource, ErrorConstant.ERROR_ClassCastException);
