...
public class InsufficientAuthenticationExceptionAdvice {
  ...
  public ResponseEntity<ResponseDTO> InsufficientAuthenticationException(InsufficientAuthenticationException ex,
                                                                         WebRequest request) {
    ...
    BizExceptionUtils.fillInStackTrace(fillInStackTrace, "InsufficientAuthenticationException", ex.fillInStackTrace().toString());

    List<ErrorMsgDTO> error = new ArrayList<ErrorMsgDTO>();

    CustomMessageDTO customMessageDTO = CustomMessageDTO.builder().build();
    BizExceptionUtils.getCustomMessage(customMessageDTO, messageSource, ErrorConstant.ERROR_InsufficientAuthenticationException);
