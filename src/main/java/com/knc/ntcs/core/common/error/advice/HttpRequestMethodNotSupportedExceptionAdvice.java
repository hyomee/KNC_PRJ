...
public class HttpRequestMethodNotSupportedExceptionAdvice {
  ...
  public ResponseEntity<ResponseDTO> HttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException ex,
    ...
    BizExceptionUtils.fillInStackTrace(fillInStackTrace, "HttpRequestMethodNotSupportedException", ex.fillInStackTrace().toString());

    List<ErrorMsgDTO> error = new ArrayList<ErrorMsgDTO>();
    CustomMessageDTO customMessageDTO = CustomMessageDTO.builder().build();
    BizExceptionUtils.getCustomMessage(customMessageDTO, messageSource, ErrorConstant.ERROR_HttpRequestMethodNotSupportedException);
    ...
    // new ResponseEntity<>(responseDTO, HttpStatus.METHOD_NOT_ALLOWED);
  }
